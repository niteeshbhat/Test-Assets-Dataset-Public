...
module.exports.processTestvector = function (item, ignoreInactive) {
    var items = [];
    var current;
    var self = this;
    var playerUrl;
    ...
    if (item.active) {
        ...
        item.testcontents.forEach(function (tc) {
            if (tc.active) {
                //if (tc.hasOwnProperty('feature') && tc.feature.active) {
                    //if (tc.feature.hasOwnProperty('featureGroup') && tc.feature.featureGroup.active) {
                        current.testcontents.push(tc.name);
        ...
        current.play = '<a target="blank" href="' + playerUrl + '?mpd=' + item.url + '">Play</a>';
        if (current.testcontents.length > 0) {
            return current;
        } else {
            return null;
        }
...
/**
 * Query all each Feature by ID, query the corresponoding testcontent and then the associated testvectors
...
var processSingleFeature = function (item) {
    ...
    Testcontent.find({
      ...
      .then(function (tcDocs) {
          ...
          return Testvector.find({testcontents: {$in: targetIds}, active: true, includeInDashjsJson: true})
            .deepPopulate("testcontents testcontents.feature testcontents.feature.featureGroup");
      })
...
module.exports.writeFeatureTestVectorJSON = function () {
    ...
    Feature.find()
      ...
      .then(function (featureDocs) {
          // gather feature, testcontent and testvector data and build up the dashJSON
